...
class AppManager {
    ...
    //Constantes de clave para UserDefaults
    ...
    private func cargarDatos() {
        ...
        //Cargar items de usuario si los tiene o estan vacios []
        if let data = UserDefaults.standard.data(forKey: claveItems),
           let itemsGuardados = try? JSONDecoder().decode([Item].self, from: data) {
...
struct SelectorDeVista: View {
    ...
    var body: some View {
        //Necesitamos un group como contenedor para aplicar el .animation a todas las vistas
        Group {
            ...
            List {
                ...
                Section("Mis articulos") {
                    if manager.items.isEmpty {
                        Text("No hay articulos guardados")
...
struct VistaOnboarding: View {
    ...
    var body: some View {
        ...
        TabView {
            crearPagina(color: .blue, titulo: "Bienvenido/a", desc: "Esta es la app de prácticas")
            crearPagina(color: .green, titulo: "Sin trampa ni cartón", desc: "Aplicación")
            crearPagina(color: .orange, titulo: "¡Empecemos!", desc: "Crea tu cuenta ahora", esUltima: true)
        }
        .tabViewStyle(.page)
    ...
    func crearPagina(color: Color, titulo: String, desc: String, esUltima: Bool = false) -> some View {
        ...
        VStack {
            ...
            Button("Acceder") {
                if nombreUsuario.isEmpty || password.isEmpty {
                    ...
                    mostrarError = false
                    manager.registrarOIniciarSesion(nombreUsuario: nombreUsuario, pass: password)
