...
class MensajeViewModel {
    ...
    func anadirGasto(texto: String,  idUsuario: Int, fecha: Date) {
        let nuevoMensaje = Mensaje(texto: texto,
                               fecha: fecha,
                               idUsuario: idUsuario
                               
                            )
        do {
            try db.collection(ConstantesFirestore.coleccionMensajes).addDocument(from: nuevoMensaje)
        } catch {
    ...
    func actualizarGasto(_ mensaje: Mensaje) {
        guard let idMensaje = mensaje.id else {return}
        do {
